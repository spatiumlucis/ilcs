...
function ReadSensorValues(white, red, green, blue){
	var target = $('.sensor_values');
	var element =   "<h3> Current Light Intensity: " + white + " lumens" + "</h3>" + 
					"<h3> Red Led Intensity: " + red + "</h3>" + 
					"<h3> Green Led Intensity: " + green + "</h3>" + 
					"<h3> Blue Led Intensity: " + blue + "</h3>";
	target.html(element);
...
function graphWhiteLight(intensity){
	var target = $('#white');
	target.html('');
	Morris.Donut({
		element: 'white',
		colors: ['#e0e0f4','#ffffff'],
  		data: [
				{value: 100 - intensity, label: ''},
    			{value: intensity, label: 'White'}
...
function graphRedLight(intensity){
	var target = $('#red');
	target.html('');
	Morris.Donut({
		element: 'red',
		colors: ['#e0e0f4','#ff0000'],
  		data: [
				{value: 100 - intensity, label: ''},
    			{value: intensity, label: 'Red'}
...
function graphGreenLight(intensity){
	var target = $('#green');
	target.html('');
	Morris.Donut({
		element: 'green',
		colors: ['#e0e0f4','#00ff00'],
  		data: [
				{value: 100 - intensity, label: ''},
    			{value: intensity, label: 'Green'}
...
function graphBlueLight(intensity){
	var target = $('#blue');
	target.html('');
	Morris.Donut({
		element: 'blue',
		colors: ['#e0e0f4','#0000ff'],
  		data: [
				{value: 100 - intensity, label: ''},
    			{value: intensity, label: 'Blue'}
...
function readSensorsValues(){
	...
	$.get(ROOM_UPDATE,
	  function(data, stat){
		  ...
		  // Degradation code
		  if(parseInt(data[5]) == 1 || parseInt(data[6]) == 1 || parseInt(data[7]) == 1 || parseInt(data[8]) == 1){
		  		$('#degrade_alert').css({'display':'block'});
				if(!alarm_on)
					$('#alarm_switch').click();
		  }

		  else{
				$('#degrade_alert').css({'display':'none'});
				if(alarm_on)
					$('#alarm_switch').click();
		  }

		  
		 /* if(graphLoaded && (whiteLight != parseInt(data[4]))){
			  ReadSensorValues(data[4], data[1], data[2], data[3]);
			  graphWhiteLight(parseInt(data[1]));
			  whiteLight = parseInt(data[4]);
		  }*/



		  if(graphLoaded && (redLight != parseInt(data[1]))){
			  ReadSensorValues(data[4], data[1], data[2], data[3]);
			  graphRedLight(parseInt(data[1]));
			  redLight = parseInt(data[1]);
			  
		  }

		  if(graphLoaded && (greenLight != parseInt(data[2]))){
			  ReadSensorValues(data[4], data[1], data[2], data[3]);
			  graphGreenLight(parseInt(data[2]));
			  greenLight = parseInt(data[2]);
		  }

		  if(graphLoaded && (blueLight != parseInt(data[3]))){
			  ReadSensorValues(data[4], data[1], data[2], data[3]);
			  graphBlueLight(parseInt(data[3]));
			  blueLight = parseInt(data[3]);
		  }


		  if(graphLoaded == 0){
			  ReadSensorValues(data[4], data[1], data[2], data[3]);
			  //graphWhiteLight(parseInt(data[4]));
			  graphRedLight(parseInt(data[1]));
			  graphGreenLight(parseInt(data[2]));
			  graphBlueLight(parseInt(data[3]));
...
function CreateRoom(roomName, roomIp, wakeTime, lightThreshold, colorThreshold){
	...
	var newDivTag  =  
		'<div class = "container rooms" id = "' + div + '">' +
				...
				'<div class = "col-sm-12">' + 
					'<div class = "col-sm-6 col-xs-12">' + 
						'<div class = "user_set_values room_info">' + 
							...
							'<h3 class = "aroom_wake_time"> Set Wake Time: ' + wakeTime + '</h3>' + 
							'<h3 class = "aroom_light_threshold"> Set Light Threshold: ' + lightThreshold + '</h3>' + 
							'<h3 class = "aroom_color_threshold"> Set Color Threshold: ' + colorThreshold + '</h3>' + 
			...
			'<div class = "row">' + 
				'<div class = "col-sm-4 col-sm-offset-4 col-xs-12">' + 
					'<div id = "degrade_alert" class = "alert alert-danger alert-dismissable">' + 
						'<a href ="#" class = "close" data-dismiss = "alert" aria-label = "close"> &times; </a>' + 
