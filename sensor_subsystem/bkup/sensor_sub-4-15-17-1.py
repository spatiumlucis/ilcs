                    ...
                    if primary_red_deg:
                        ...
                        if secondary_red_on:
                            if secondary_red_degraded:
                                ...
                                try:
                                    primary_red_comp = circadian_red + (circadian_red - red2)
                    ...
                    if primary_green_deg:
                        ...
                        if secondary_green_on:
                            if secondary_green_degraded:
                                ...
                                primary_comp_mutex.acquire()
                                try:
                                    primary_green_comp = circadian_green + (circadian_green - green2)
                    ...
                    if primary_blue_deg:
                        ...
                        if secondary_blue_on:
                            if secondary_blue_degraded:
                                ...
                                primary_comp_mutex.acquire()
                                try:
                                    primary_blue_comp = circadian_blue + (circadian_blue - blue2)
    ...
    while True and keyboard_Event.isSet():
        ...
        if sleep_mode == False and change_par == False:
            ...
            try:
                ...
                #     PREV_COLORS[0] = 0 #USER_CIRCADIAN_TABLE[current_minute][0]
                # else:
                #     PREV_COLORS[0] = USER_CIRCADIAN_TABLE[current_minute][0]
                # if USER_CIRCADIAN_TABLE[current_minute][1] == 0:
                #     PREV_COLORS[1] = 0 #USER_CIRCADIAN_TABLE[current_minute][1]
                # else:
                #     PREV_COLORS[1] = USER_CIRCADIAN_TABLE[current_minute][1]
                # if USER_CIRCADIAN_TABLE[current_minute][0] == 0:
                #     PREV_COLORS[2] = 0 #USER_CIRCADIAN_TABLE[current_minute][2]
                # else:
                #     PREV_COLORS[2] = USER_CIRCADIAN_TABLE[current_minute][2]
                if primary_red_deg:
                    PREV_COLORS[0] = red_comp
                else:
                    PREV_COLORS[0] = USER_CIRCADIAN_TABLE[current_minute][0]

                if primary_green_deg:
                    PREV_COLORS[1] = green_comp
                else:
                    PREV_COLORS[1] = USER_CIRCADIAN_TABLE[current_minute][2]

                if primary_blue_deg:
                    PREV_COLORS[2] = blue_comp
                else:
                    PREV_COLORS[2] = USER_CIRCADIAN_TABLE[current_minute][2]
                #PREV_COLORS[0] = USER_CIRCADIAN_TABLE[current_minute][0]
                #PREV_COLORS[1] = USER_CIRCADIAN_TABLE[current_minute][1]
