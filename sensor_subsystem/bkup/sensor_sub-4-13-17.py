...
def init_red_offset():
    ...
    while t < 1440:
        if t >= 313 and t <= 329:
            MASTER_OFFSET_TABLE[t][0] = 0.0625 * t - 19.56
        elif t >= 329 and t <= 333:
            MASTER_OFFSET_TABLE[t][0] = 0.25 * t - 81.25
        elif t >= 333 and t <= 337:
            MASTER_OFFSET_TABLE[t][0] = 0.5 * t - 164.5
        elif t >=337 and t<= 343:
            MASTER_OFFSET_TABLE[t][0] = 0.33 * t - 108.33
        elif t >= 343 and t <= 373:
            MASTER_OFFSET_TABLE[t][0] = 0.5 * t - 165.5
        elif t >= 373 and t <= 379:
            MASTER_OFFSET_TABLE[t][0] = 0.33 * t - 103.3
        elif t >= 379 and t <= 381:
            MASTER_OFFSET_TABLE[t][0] = t - 356
        elif t >= 381 and t <= 401:
            MASTER_OFFSET_TABLE[t][0] = 0.5 * t - 165.5
        elif t >= 401 and t <= 403:
            MASTER_OFFSET_TABLE[t][0] = t - 366
        elif t >= 403 and t <= 411:
            MASTER_OFFSET_TABLE[t][0] = 0.5 * t - 164.5
        elif t >= 411 and t <= 413:
            MASTER_OFFSET_TABLE[t][0] = t - 370
        elif t >= 413 and t <= 419:
            MASTER_OFFSET_TABLE[t][0] = 0.66 * t - 232.3
        elif t >= 419 and t <= 420:
            MASTER_OFFSET_TABLE[t][0] = t - 372
        elif t >= 420 and t <= 720:
            MASTER_OFFSET_TABLE[t][0] = (89.0/300) * t - 76.6
        elif t >= 720 and t <= 1140:
            MASTER_OFFSET_TABLE[t][0] = (-8.0 / 420) * t + 150.7
        t += 1
...
def init_green_offset():
    ...
    while t < 1440:
        if t >= 313 and t <= 319:
            MASTER_OFFSET_TABLE[t][1] = 0.25 * t - 78.25
        elif t >= 319 and t <= 321:
            MASTER_OFFSET_TABLE[t][1] = 0.5 * t - 157.5
        elif t >= 321 and t <= 325:
            MASTER_OFFSET_TABLE[t][1] = 0.25 * t - 77.25
        elif t >= 325 and t <= 337:
            MASTER_OFFSET_TABLE[t][1] = 0.5 * t - 158.5
        ...
        elif t >= 395 and t <= 401:
            MASTER_OFFSET_TABLE[t][1] = t - 332
        elif t >= 401 and t <= 405:
            MASTER_OFFSET_TABLE[t][1] = 1.25 * t - 432.25
        elif t >= 405 and t <= 407:
            MASTER_OFFSET_TABLE[t][1] = 1.5 * t - 533.5
        elif t >= 407 and t <= 411:
            MASTER_OFFSET_TABLE[t][1] = 0.75 * t - 228.25
        elif t >= 411 and t <= 413:
...
def init_blue_offset():
    ...
    while t < 1440:
        if t >= 313 and t <= 323:
            MASTER_OFFSET_TABLE[t][2] = 0.5 * t - 156.5
        ...
        elif t >= 351 and t <= 355:
            MASTER_OFFSET_TABLE[t][2] = 1.25 * t - 408.75
        elif t >= 355 and t <= 357:
            MASTER_OFFSET_TABLE[t][2] = 1.5 * t - 497.5
        elif t >= 357 and t <= 363:
            MASTER_OFFSET_TABLE[t][2] = 2 * t - 676
        elif t >= 363 and t <= 367:
            MASTER_OFFSET_TABLE[t][2] = t - 313
            ...
            MASTER_OFFSET_TABLE[t][2] = 1.5 * t - 479.5
        elif t >= 401 and t <= 403:
            MASTER_OFFSET_TABLE[t][2] = 2 * t - 680
            ...
            MASTER_OFFSET_TABLE[t][2] = 1.75 * t - 577.25
        elif t >= 411 and t <= 413:
            MASTER_OFFSET_TABLE[t][2] = 2 * t - 680
        elif t >= 413 and t <= 415:
        ...
        elif t >= 417 and t <= 420:
            MASTER_OFFSET_TABLE[t][2] = 1.67 * t - 537
        elif t >= 420 and t <= 720:
            pass
        elif t >= 720 and t <= 1140:
            pass
    ...
    while True and keyboard_Event.isSet():
        ...
        if sleep_mode == False and change_par == False:
            ...
            try:
                ...
                if USER_CIRCADIAN_TABLE[current_minute][0] == 0:
                    PREV_COLORS[0] = USER_CIRCADIAN_TABLE[current_minute][0]
                else:
                    PREV_COLORS[0] = USER_CIRCADIAN_TABLE[current_minute][0]
                if USER_CIRCADIAN_TABLE[current_minute][1] == 0:
